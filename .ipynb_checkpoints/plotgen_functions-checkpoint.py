...
def rescaled_disk(m=1.0):
    ...
    for qr in Qr:
        ...
        for qz in Qz:
            q = np.array([qr,qz])
            f = max_f(m, 100, q, rho_mirrordisk, iso = False)
            f_array[ir,iz] = f
            iz = iz+1
        ir = ir+1
...
def DD_mass_constraints(q=1.0):
    ...
    j = 0
    for m in M:
        logf = max_f(m, 100, qa, rho_mirrordisk, logfexp = 0, iso = False, baryons=True) #use the new function
...
def tiltdisk(m=1.0):
    ...
    Phis = np.linspace(np.pi*5/180, np.pi*45/180, 9)
    Thetas = np.arange(0,2*np.pi, np.pi/2)

    result_array_tilt = np.zeros([9,4])
    ...
    for theta in Thetas:
        ...
        for phi in Phis:
            q = [theta,phi]
            n_50 = max_f(m,100, q, rho_mirrordisk_tilt, iso=False)
            result_array_tilt[i,j] = n_50
            print('tiltdisk finished for ', theta, phi, n_50)
            i = i+1
        j = j+1

    rat0 = max_f(m, 100, [0,0], rho_mirrordisk_tilt, iso=False)
...
def n_los(m, q, rho, name, rand = False):
    ...
    np.savetxt('n_LoS_m{}.txt'.format(name), f_array)
    
    
def n_los_rand(m, q, rho, name, rand = False):
    
    N = np.array([5, 10, 20, 50, 100, 200, 500])#1000, 2000])
    
    f_array = np.zeros([2+7*5])
